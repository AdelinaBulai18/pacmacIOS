...
-(id)initWithSize:(CGSize)size {    
    if (self = [super initWithSize:size]) {
        ...
        manager = [[GameManager alloc] initWithGameId:@"TestFlappyBirdGame"];
        
        if([[manager HighScore] intValue] == 0){
            [manager setHighScore:0];
        }
        
        _highscoreLabelNode = [SKLabelNode labelNodeWithFontNamed:@"MarkerFelt-Wide"];
        _highscoreLabelNode.fontSize = 10.00;
        _highscoreLabelNode.position = CGPointMake( 35, self.frame.size.height -80 );
        _highscoreLabelNode.zPosition = 100;
        _highscoreLabelNode.text = [NSString stringWithFormat:@"Highscore: %d", [[manager HighScore] intValue]];
        [self addChild:_highscoreLabelNode];
        
        
        //user name label
        
        _userLabelNode = [SKLabelNode labelNodeWithFontNamed:@"MarkerFelt-Wide"];
        _userLabelNode.fontSize = 10.00;
        _userLabelNode.zPosition = 100;
        _userLabelNode.text = [NSString stringWithFormat:@"User: %@", [manager UserName]];
        _userLabelNode.position = CGPointMake(self.frame.size.width-(_userLabelNode.frame.size.width), self.frame.size.height -80 );
        [self addChild:_userLabelNode];
    }
    return self;
}
...
- (void)didBeginContact:(SKPhysicsContact *)contact {
    if( _moving.speed > 0 ) {
        if( ( contact.bodyA.categoryBitMask & scoreCategory ) == scoreCategory || ( contact.bodyB.categoryBitMask & scoreCategory ) == scoreCategory ) {
            ...
            if (_score > [[manager HighScore] intValue]){
                [manager setHighScore: [NSNumber numberWithInt: _score]];
                _highscoreLabelNode.text = [NSString stringWithFormat:@"Highscore: %@", [manager HighScore]];
            }
